...
4. **タグ付き PDF の上流接続（foliojet）** — ✅ v1 実装済み（2026-07-12）
   ...
   **`_9520_UA/PdfUaValidationTest` が veraPDF で PDF/UA-1 適合を検証**（見出し・
   段落・リスト・表を含む文書＋フォーム入力を含む文書、埋め込みフォント使用）。
   `_9500_PROFILE` でも `/S /H1`・`/P`・`/L`・`/Table` 等をバイト検証。残: ページ
   またぎ要素の単一化（現状ページ毎に分割）、フォームの `Form` 要素を包含ブロックの
   読み順位置へ（現状は文書直下）、リンクのインライン構造、Figure の alt、より広い
   要素マッピング。
